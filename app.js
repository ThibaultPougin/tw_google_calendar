...
const teamwork_controller = require('./src/controllers/teamwork_controller');
const cli_controller = require('./src/controllers/cli_controller');
const global_controller = require('./src/controllers/global_controller');

const tags_file = fs.readFileSync('./config/tags.json');
const all_tags = JSON.parse(tags_file);

const synchro_tw_google_agenda = {
    ...
    start: async () => {
        ...
        // Si des événements sans tâche Teamwork associée sont présents, on démarre le CLI interactif
        cli_controller.handle_interactive_command_line(total_log_hours, total_unlog_hours, unlog_events);

    }
}

synchro_tw_google_agenda.start();